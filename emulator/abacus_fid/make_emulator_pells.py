...
fsig8_ref = pkclass.scale_independent_f_sigma8(z)

#save Plin for test purposes
h=0.6736
sig8_z = pkclass.sigma(8,z,h_units=True)
ki = np.logspace(-3.0,1.0,200)
pi = np.array( [pkclass.pk_cb(k*h, z ) * h**3 for k in ki] )
plin_dat = np.zeros((len(ki),3))
plin_dat[:,0] = ki
plin_dat[:,1] = pi
plin_dat[0,2] = sig8_z
plin = np.matrix(plin_dat)
np.savetxt('./Plin_z{:.2f}.txt'.format(z),plin,encoding='utf-8')
...
x0s = [fsig8_ref, 1.0,1.0,0.0]; Nparams = len(x0s) # these are chosen to be roughly at the BOSS best fit value
