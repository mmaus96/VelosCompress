...
def compute_pell_tables(pars,pkclass, z=0.61,fid_dists= None, ap_off=False ):
    
    # OmegaM, h, sigma8 = pars
    # Hzfid, chizfid = fid_dists
    f_sig8,apar,aperp,m = pars

    Plin_dat = np.loadtxt('/global/cscratch1/sd/mmaus/ShapeFit/emulator/abacus_fid/emu/Plin.txt')
    ki = Plin_dat[:,0]
    pi = Plin_dat[:,1]
    sig8_z = Plin_dat[0,2]
    ...
    pi = pi*np.exp( shapefit_factor(ki,m) )  #np.array( [pkclass.pk_cb(k*h, z ) * h**3 for k in ki] ) * np.exp( shapefit_factor(ki,m) )
